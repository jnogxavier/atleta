...
RSpec.describe Student::PasswordController, type: :controller do
  after do
    Current.reset
  end
  ...
  describe 'authentication and authorization' do
    context 'when user is not authenticated' do
      ...
      it 'requires authentication for update' do
        patch :update, params: { user: { current_password: 'old', password: 'new', password_confirmation: 'new' } }
        expect(response).to have_http_status(:redirect)
      end
    end
  end
  ...
  describe 'controller structure' do
    ...
    it 'is part of the Student module' do
      expect(described_class.name).to eq('Student::PasswordController')
    end
  ...
  describe 'parameter handling' do
    it 'permits password parameters' do
      params = ActionController::Parameters.new(user: {
        current_password: 'old123',
        password: 'new123',
        password_confirmation: 'new123',
        email: 'test@test.com' # Not permitted
      })
      # Can't test private method parameter filtering without stubbing request
